...
include_once('c:/xampp/htdocs/Watches/config.php');
...
if (isset($_POST['update_user'])) {
    ...
    $image = mysqli_real_escape_string($config, $_POST['image']);
    // $imagePath2 = mysqli_real_escape_string($config, $_POST['imagePath2']);
    $title = mysqli_real_escape_string($config, $_POST['name']);
    $price = mysqli_real_escape_string($config, $_POST['price']);
    // $dRate = mysqli_real_escape_string($config, $_POST['deletedRate']);

    // Prepare and execute the UPDATE query

    $query = "UPDATE newarrival SET img='$image',  title='$title',price='$price' WHERE id='$userId'";
